    ...
    <div class="write-contract white-box padding-align-40">
        ...
        <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="155px">
            ...
            <el-form-item label="合同图片:" prop="img">
                <img-upload :imgs="ruleForm.img"
                            :max="3"
                            remark="请上传合同复印件或实拍清晰图片，大小不超过2M"
                            @change="changeImg"/>
            </el-form-item>
        </el-form>
        <div class="bottom-btn">
            <el-button @click="goBack()">返回上一页</el-button>
            <el-button type="primary" @click="submitForm('ruleForm')">保存合同</el-button>
        </div>
        <!-- 合同状态 -->
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            // 图片改变，新增，删除
            changeImg(img) {
                this.$set(this.ruleForm, 'img', img)
            },
            // 提交保存
            submitForm(formName) {
                ...
                if (valid) {
                    if(!this.companyActive || !this.ruleForm.teamActive) {
                        ...
                        .then(() => {
                            let imgsDownCode = []
                            this.ruleForm.img.map(item => {
                                imgsDownCode.push(item.downCode)
                            })
                            // 企业id
                            ...
                            let params = {
                                ...
                                contractAddress:      imgsDownCode.join(',')
                            }
